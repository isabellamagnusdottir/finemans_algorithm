    ...
    while not pq.empty():
        ...
        for v in graph[u]:
            ...
            if alt_dist < dist[v]:
                ...
                if save_source:
                    _compute_ancestor_parent(parent, anc_in_I, I_prime, u, v, len(graph)-1)
    return dist


# hold styr på hvorvidt vægten, som står der nu, kommer fra den nuværende bellman-ford eller fra tidligere
def bellman_ford(graph : dict[int, dict[int, int]], neg_edges: set, dist: list, I_prime = None, parent = None, anc_in_I = None, save_source = False):
...
def bfd(graph, neg_edges, dist: list, beta: int, I_prime = None, parent = None, anc_in_I = None, save_source = False):
    ...
    dist = dijkstra(graph, neg_edges, dist, I_prime=I_prime, parent=parent, anc_in_I=anc_in_I, save_source=save_source)
    for _ in range(beta):
        dist = bellman_ford(graph, neg_edges, dist, I_prime=I_prime, parent=parent, anc_in_I=anc_in_I, save_source=save_source)
        dist = dijkstra(graph, neg_edges, dist, I_prime=I_prime, parent=parent, anc_in_I=anc_in_I, save_source=save_source)
    return dist
...
def b_hop_sssp(source, graph: dict[int, dict[int, int]], neg_edges: set, beta, I_prime = None, parent = None, anc_in_I = None, save_source = False):
    ...
    return bfd(graph, neg_edges, dist, beta, I_prime=I_prime, parent=parent, anc_in_I=anc_in_I, save_source=save_source)
...
def _subset_bfd(graph, neg_edges, subset, beta, save_source = False):
    ...
    distances = b_hop_sssp(super_source, graph, neg_edges, beta, I_prime=subset, parent=parent, anc_in_I=anc_in_I, save_source=save_source)
...
def subset_bfd(graph: dict[int, dict[int, int]], neg_edges: set[tuple[int, int]], subset: set[int], beta: int, save_source = False):
    return _subset_bfd(graph, neg_edges, subset, beta, save_source = save_source)[:-1]

# TODO: consider refactoring cycle detection
def super_source_bfd(graph: dict[int, dict[int, int]], neg_edges: set[tuple[int, int]], beta, cycleDetection = False):
...
def compute_reach(graph: dict[int, dict[int, int]], neg_edges: set[tuple[int, int]], subset: set[int] , h: int):
