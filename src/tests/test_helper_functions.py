...
def test_dijkstra_implementation(filename, expected):
    ...
    initial_dist = [inf]*(len(graph.keys()))
    initial_dist[0] = 0
...
def test_dijkstra_and_bellman_ford_manually_with_save_ancestors():
    graph, neg_edges = load_test_case(TESTDATA_FILEPATH + "graph_with_neg_edges.json")
    super_source = len(graph)
    graph[super_source] = {}
    actual_distances = [inf] * len(graph)
    actual_distances[super_source] = 0

    I_prime = {0, 5, 12}
    for i in I_prime:
        graph[super_source][i] = 0

    parent = [nan] * len(graph.keys())
    anc_in_I = [nan] * len(graph.keys())

    expected_distances = [0, 3, 14, 7, inf, 0, 5, inf, inf, 9, inf, inf, 0, 8]
    actual_distances = dijkstra(graph, neg_edges, actual_distances, I_prime = I_prime, anc_in_I = anc_in_I, parent = parent, save_source = True)
    assert actual_distances[:-1] == expected_distances

    expected_parents = [nan, 0, 6, 0, nan, nan, 5, nan, nan, 5, nan, nan, nan, 6]
    expected_ancestors = [nan, 0, 5, 0, nan, nan, 5, nan, nan, 5, nan, nan, nan, 5]
    assert parent[:-1] == expected_parents
    assert anc_in_I[:-1] == expected_ancestors

    expected_distances = [0, 3, -6, 7, 5, 0, 5, 1, 2, 9, inf, inf, 0, 5]
    actual_distances = bellman_ford(graph, neg_edges, actual_distances, I_prime = I_prime, anc_in_I = anc_in_I, parent = parent, save_source = True)
    assert actual_distances[:-1] == expected_distances

    expected_parents = [nan, 0, 0, 0, 3, nan, 5, 1, 6, 5, nan, nan, nan, 9]
    expected_ancestors = [nan, 0, 0, 0, 0, nan, 5, 0, 5, 5, nan, nan, nan, 5]
    assert parent[:-1] == expected_parents
    assert anc_in_I[:-1] == expected_ancestors

    expected_distances = [0, 3, -6, 7, 5, 0, 5, 1, 2, 9, 4, 6, 0, 5]
    actual_distances = dijkstra(graph, neg_edges, actual_distances, I_prime = I_prime, anc_in_I=anc_in_I, parent = parent, save_source = True)
    assert actual_distances[:-1] == expected_distances

    expected_parents = [nan, 0, 0, 0, 3, nan, 5, 1, 6, 5, 8, 7, nan, 9]
    expected_ancestors = [nan, 0, 0, 0, 0, nan, 5, 0, 5, 5, 5, 0, nan, 5]
    assert parent[:-1] == expected_parents
    assert anc_in_I[:-1] == expected_ancestors

    expected_distances = [0, 3, -6, 7, -7, 0, 5, 1, -11, 9, 4, 6, 0, 5]
    actual_distances = bellman_ford(graph, neg_edges, actual_distances, I_prime=I_prime, anc_in_I=anc_in_I, parent=parent, save_source=True)
    assert actual_distances[:-1] == expected_distances

    expected_parents = [nan, 0, 0, 0, 2, nan, 5, 1, 2, 5, 8, 7, nan, 9]
    expected_ancestors = [nan, 0, 0, 0, 0, nan, 5, 0, 0, 5, 5, 0, nan, 5]
    assert parent[:-1] == expected_parents
    assert anc_in_I[:-1] == expected_ancestors

    expected_distances = [0, 3, -6, 7, -7, 0, -5, 1, -11, 9, -9, 6, -2, -2]
    actual_distances = dijkstra(graph, neg_edges, actual_distances, I_prime=I_prime, anc_in_I=anc_in_I, parent=parent, save_source=True)
    assert actual_distances[:-1] == expected_distances

    expected_parents = [nan, 0, 0, 0, 2, nan, 10, 1, 2, 5, 8, 7, 10, 6]
    expected_ancestors = [nan, 0, 0, 0, 0, nan, 0, 0, 0, 5, 0, 0, 0, 0]
    assert parent[:-1] == expected_parents
    assert anc_in_I[:-1] == expected_ancestors


def test_subset_bfd_with_save_ancestors():
    graph, neg_edges = load_test_case(TESTDATA_FILEPATH + "graph_with_neg_edges.json")
    I_prime = {0, 5, 12}
    actual_distances = subset_bfd(graph, neg_edges, I_prime,1, save_source=True)

    expected_distances = [0, 3, -6, 7, 5, 0, 5, 1, 2, 9, 4, 6, 0, 5]
    assert actual_distances == expected_distances

def test_subset_bfd_with_save_ancestors_on_one_hop_cycle_detects_cycle():
    graph, neg_edges = load_test_case(TESTDATA_FILEPATH + "graph_with_one_hop_cycle.json")
    I_primes = [{0,4}, {1,4}, {3,4}]

    for set in I_primes:
        with pytest.raises(NegativeCycleError):
            subset_bfd(graph, neg_edges, set,1, save_source=True)


def test_subset_bfd_with_save_ancestors_on_two_hop_cycle_does_not_detect_cycle():
    graph, neg_edges = load_test_case(TESTDATA_FILEPATH + "graph_with_neg_cycle.json")
    I_prime = {0, 5, 12}
...
def test_subset_bfd_for_only_pos_edges():
    subset = {0}
...
def test_reach_on_positive_path():
    graph,neg_edges = load_test_case(TESTDATA_FILEPATH + "path_with_only_positive_edges.json")
    reach = compute_reach(graph, neg_edges, {0}, 10)
    assert reach == set()

def test_independent_set_negative_cycle_with_insufficient_hops():
    subset = {0}
    expected = [0,-1,inf,inf]
    graph,neg_edges = load_test_case(TESTDATA_FILEPATH + "negative_cycle_4.json")
    actual = subset_bfd(graph, neg_edges, subset, 1, save_source=True)
    assert actual == expected

def test_independent_set_negative_cycle_detected():
    subset = {0}
    graph,neg_edges = load_test_case(TESTDATA_FILEPATH + "small_alternating_cycle.json")
    with pytest.raises(NegativeCycleError):
        subset_bfd(graph, neg_edges, subset, 1, save_source=True)

def test_independent_set_negative_cycle_not_detected():
    subset = {0}
    expected_distances = [0, 3, -6, 7, 5, 11, 8, 1, 2, 20, 4, 6, 11, 11]

    graph,neg_edges = load_test_case(TESTDATA_FILEPATH + "graph_with_neg_edges.json")
    actual = subset_bfd(graph, neg_edges, subset, 1, save_source=True)
    assert actual == expected_distances

def test_independent_set_negative_cycle_not_detected_small():
