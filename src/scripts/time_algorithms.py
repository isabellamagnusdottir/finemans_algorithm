...
def time_algorithms():
    ...
    files = [filename for filename in os.listdir(GRAPHS_PATH) if "DS_Store" not in filename]

    files = sorted(files, key=lambda x: (x.split("_")[0], int(x.split("_")[1]), int(x.split("_")[2])))
